...
import Prosolid from "../../../assets/logo.webp";
...
const Navrrss = ({ politica, altura }) => {
    const url = window.location.href;

    return (
        <nav
            id="navRRSS"
            className={`p-2 flex flex-row justify-between items-center bg-white md:h-${altura}`}
        >
            <div>
                {politica && (
                    <p className="mobile:text-[8px] sm:text-[14px]">
                        <a className="hover:text-red transition-all ml-10 mobile:ml-0" href={`${url}politica-de-privacidad`}>
            ...
            <div id="rrss" className={`flex flex-row items-center mr-4 mobile:m-0 `}>
                ...
                <div className="flex flex-row mobile:mr-8">
                    <div id="logo" className="flex items-center ml-8">
                        <a target="_blank" href="https://prosolid3d.com/" rel="noopener noreferrer">
                            <img className="mobile:w-24 sm:w-44" src={Prosolid} alt="prosolid" />
                        </a>
                    </div>
                </div>
            </div>
